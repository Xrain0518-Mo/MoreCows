...
local function GenModelBlockItem()
	...
	outFile:write('{\n');
	local content = string.format("\t\"parent\": \"%s:blockBush/%s\"\n", modName,blockName );
...
local function GenModelBlock()
	local path = string.format("src\\main\\resources\\assets\\%s\\models\\blockBush\\%s.json", modName, blockName);
	outFile = io.open(path,"w");
	outFile:write('{\n');
	outFile:write('\t\"parent\": \"blockBush/cube_all\",\n');
	outFile:write('\t\"textures\": {\n');
	local content = string.format("\t\t\"all\": \"%s:blockBush/%s\"\n", modName,blockName );
	outFile:write(content);
	outFile:write('\t}\n');
	outFile:write('}\n');
	outFile:close();
